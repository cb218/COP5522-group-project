...
void oe_sort_seq(int *numbers, int array_length, int Nthreads) {
    int phase, idx, tmp;
    #pragma omp parallel private (phase) num_threads(Nthreads)
...
int main(int argc, char **argv) {
    ...
    oe_sort_seq(numbers, array_length, num_cores);
    gettimeofday(&end, 0);
